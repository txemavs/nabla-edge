...
class OledShell:
    ...
    def _render_config_menu(self):
        ...
        if self.state.action_message:
            ...
            draw.rectangle(
                [box_x, box_y, box_x + box_w, box_y + box_h], fill=0, outline=1
            )
...
ACTION_MAP: dict[str, list[str]] = {
    ...
    "voice_status": [
        ...
        "compose",
        "-f",
        "/opt/nabla-edge/voice/lva/docker-compose.yml",
        "ps",
    ],
    "voice_start": [
        "docker",
...
class EncoderInput:
    """EC11 rotary via RPi.GPIO with quadrature full-step state machine + debounce.

    Bounce often fires CW then CCW (or vice versa) in quick succession ("pipom").
    A 2ms bouncetime is too weak for noisy encoders.

    This implementation:
    - Uses a full-step quadrature state machine that only emits on valid
      4-transition sequences (one per detent)
    - Applies hardware bouncetime=5ms on both CLK and DT edges
    - Enforces 80ms software debounce between accepted steps
    - SW press uses 300ms debounce with 20ms settle read
    """

    # Full-step quadrature state machine: (prev_state << 2 | new_state) -> direction
    # Only valid full-step transitions produce output (+1 CW, -1 CCW)
    _FULLSTEP = {
        0b0001: 1,
        0b0111: 1,
        0b1110: 1,
        0b1000: 1,  # CW sequence
        0b0010: -1,
        0b1011: -1,
        0b1101: -1,
        0b0100: -1,  # CCW sequence
    }
    ...
    def __init__(self, on_rotate: Callable[[int], None], on_press: Callable[[], None]):
        ...
        self._debounce_rotate = 0.08  # 80ms between accepted rotation steps
        self._debounce_press = 0.30  # 300ms between accepted presses
        self._lock = threading.Lock()
        ...
        # Listen on BOTH edges for CLK and DT with hardware bouncetime
        GPIO.add_event_detect(
            PIN_CLK, GPIO.BOTH, callback=self._edge_callback, bouncetime=5
        )
        GPIO.add_event_detect(
            PIN_DT, GPIO.BOTH, callback=self._edge_callback, bouncetime=5
        )
        # SW uses FALLING with its own bouncetime
        GPIO.add_event_detect(
            PIN_SW, GPIO.FALLING, callback=self._sw_callback, bouncetime=100
        )

    def _edge_callback(self, channel: int) -> None:
        """Handle CLK or DT edge transition via quadrature state machine."""
        ...
        with self._lock:
            ...
            delta = self._FULLSTEP.get(key)
            self._state = new_state

            if delta is None:
                # Invalid or intermediate transition — ignore
                return

            now = time.time()
            if now - self._last_rotate_time < self._debounce_rotate:
                # Too soon after last accepted step — suppress
                return
            self._last_rotate_time = now

        # Call handler outside lock to avoid blocking GPIO interrupts
        self.on_rotate(delta)

    def _sw_callback(self, channel: int) -> None:
        """Handle switch press with debounce and settle verification."""
        now = time.time()
        if now - self._last_press_time < self._debounce_press:
            return

        # Wait for contact to settle, then verify still pressed
        time.sleep(0.02)
